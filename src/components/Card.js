...
export default function Cards({url,title,desc}){
    return(
        <div class="max-w-sm bg-white border mx-5 my-5 border-gray-200 rounded-lg shadow rounded-lg dark:bg-gray-800 dark:border-gray-700">
        <Link href="#">
            <img class="rounded-t-md" src={url} alt="" />
        </Link>
        <div class="p-5 items-center text-center">
            <Link href="#">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{title}</h5>
            </Link>
            <p class="mb-3 font-semibold text-gray-700 dark:text-gray-400">{desc}</p>
